...
SC.names="A";chr.n=1;upper.maf=0.25;lower.maf=0;out.name="test";tmp.dir="test";rep.i=1
...
if(is.na(total$nMAF.Below) == "TRUE") {
         ...
         # Compile
         dt <- data.table(nucdiv, nucdiv2, tajimad,  chrom[chr.n], rep.i)
         names(dt) <- c("tP.stan.win", "tW.stan.win", "tajima.D.win", "chr", "rep")
         
         # Windows in Mb
         dt[, winMb:=seq(from=0.01, to=leg/100, by=0.01)]
         
         # Combine
         total <- total %>% full_join(dt, by = c("rep", "chr"))
