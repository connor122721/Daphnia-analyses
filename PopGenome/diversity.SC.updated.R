# Nucleotide diversity statistics from VCF/BCF
# 12.13.2020
...
upper.maf <- as.numeric(arg[3])
lower.maf <- as.numeric(arg[4])
out.name <- as.character(arg[5])
tmp.dir <- as.character(arg[6])

SC.names="OO";chr.n=1;upper.maf=0.25;lower.maf=0;out.name="test";tmp.dir="test"
...
message(paste("SC :", SC.names, "Chromosome", chr.n,  sep=" "))
      
# Load tabindex vcf file - load in by chromosome
vcf <- quiet(readVCF(filename = "vcf/MapJune2020_ann.hyrbid_strategy.3species.whatshap.shapeit.vcf.gz", 
         tid = c(chrom[chr.n]),
         samplenames = fread("scripts/wildtrust.samples", header = F)$V1,
         frompos = 1, 
...
if(is.na(total$nMAF.Below) == "TRUE") {
	 ...
	 # Split into 250 kb windows
         slide <- sliding.window.transform(genofile,  
                                           width = 250000, 
                                           jump = 50000, 
                                           type = 2)
         
         # Total number of windows
         leg <- length(slide@region.names)
         
         # Diversity statistics
         slide <- neutrality.stats(slide)
         
         # Extract stats
         nucdiv <- slide@theta_Tajima
         nucdiv2 <- slide@theta_Watterson
         tajimad <- slide@Tajima.D
         
         # Normalized by the number of sites within each window
         nucdiv <- nucdiv/250000
         nucdiv2 <- nucdiv2/250000
         
         # Compile
         dt <- data.table(nucdiv, nucdiv2, tajimad,  chrom[chr.n])
         names(dt) <- c("tP.stan.win", "tW.stan.win", "tajima.D.win", "chr")
         
         # Windows in Mb
         dt[, winMb:=slide@region.names]
         
         # Combine
         total <- total %>% full_join(dt, by = c("chr"))
