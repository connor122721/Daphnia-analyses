...
data <- na.omit(read.csv("C:/Users/Conno/Desktop/Fall2020/Berglandlab/PopGenome/Data/100rep.new.15samp.diversity.csv",
                            header = F))
colnames(data) <- c("tW", "tW.stan", "tP", "tP.stan", "tP.eff", "tajimaD", "chr", "n.tot.sites", "denom.pi",
                    "nMAF.above", "nMAF.below", "bedSites", "biall.Sites", "sites.inc", "n.samp", "up.maf", 
                    "low.maf", "SC", "rep")

data <- data.table(data %>% select(tW.stan, tP.stan, tajimaD, SC) %>% mutate(tP.stan=as.numeric(tP.stan),
                                                              tW.stan=as.numeric(as.character(tW.stan)),
                                                              tajimaD=as.numeric(tajimaD),
                                                              SC=SC))

# Diversity long format
nuc.div1 <- data.table(data[,list(avg.pi=mean(tP.stan, na.rm=T),
                      lci.p=quantile(tP.stan, 0.025, na.rm=T), 
                      uci.p=quantile(tP.stan, 0.975, na.rm=T),
                      avg.watterson=mean(tW.stan, na.rm=T),
                      lci.w=quantile(tW.stan, 0.025, na.rm=T), 
                      uci.w=quantile(tW.stan, 0.975, na.rm=T),
                      avg.tajima=mean(tajimaD, na.rm=T),
                      lci.tajima=quantile(tajimaD, 0.025, na.rm=T), 
                      uci.tajima=quantile(tajimaD, 0.975, na.rm=T)),
                list(SC)])
...
sim <- data.table(read.csv("C:/Users/Conno/Desktop/Fall2020/Berglandlab/Data/constant.test.output.csv"))
